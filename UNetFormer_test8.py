...
# --- 1. 几何先验核心模块 (来自 DFormerv2) ---
class GeoPrior(nn.Module):
    def __init__(self, embed_dim=256, num_heads=8, initial_value=2, heads_range=6):
        super().__init__()
        # 生成角度编码参数
    ...
    def generate_2d_depth_decay(self, depth_grid):
        '''生成基于深度的衰减 mask'''
        B, _, H, W = depth_grid.shape
        grid_d = depth_grid.reshape(B, H*W, 1)
        mask_d = grid_d[:, :, None, :] - grid_d[:, None, :, :]
        mask_d = (mask_d.abs()).sum(dim=-1) # (B, L, L)
        return mask_d

    def forward(self, x_feat, depth_map):
        '''
        x_feat: (B, C, H, W) 用于获取尺寸
        depth_map: (B, 1, H_in, W_in) 原始深度图/DSM
        '''
        B, C, H, W = x_feat.shape
        # 1. 将 Depth 下采样到 Feature Map 的尺寸 (例如 16x16)
        depth_map = F.interpolate(depth_map, size=(H, W), mode='bilinear', align_corners=False)
        depth_map = depth_map.float()

        # 2. 生成 Mask
        mask_pos = self.generate_pos_decay(H, W) # (L, L)
        mask_depth = self.generate_2d_depth_decay(depth_map) # (B, L, L)

        # 3. 融合 Mask (DFormerv2 的核心公式)
        # 这一步计算 Bias: G = G_pos * G_depth
        # Log space: log(G) = log(G_pos) + log(G_depth) + decay_param
        # 为了数值稳定性，通常操作是在 log 域或者是加权求和
        # 这里简化复现原论文逻辑：Weighted Sum applied to Decay
        
        # 注意：这里我们返回 Bias 项，用于加在 Attention Score 上
        # Bias Shape: (B, Num_Heads, L, L)
        
        mask_pos = mask_pos.unsqueeze(0).unsqueeze(0) # (1, 1, L, L)
        mask_depth = mask_depth.unsqueeze(1)          # (B, 1, L, L)
        
        # (B, Heads, 1, 1)
        decay_weight = self.decay.view(1, self.num_heads, 1, 1)
        
        # 简单的线性组合作为几何引导 (你可以根据论文调整系数)
        # 越大的差异(mask值大)，衰减越强(负值越小)
        # geometric_bias = (0.5 * mask_pos + 0.5 * mask_depth) * decay_weight
        geometric_bias = (mask_pos + mask_depth) * decay_weight
        
        return geometric_bias

# --- 2. 几何感知辅助头 (Auxiliary Head) ---
class GeoAwareAuxHead(nn.Module):
    def __init__(self, in_channels=256, num_classes=6, embed_dim=256, num_heads=8):
        super().__init__()
        self.num_heads = num_heads
        self.scale = (embed_dim // num_heads) ** -0.5
        
        # 几何先验计算模块
        self.geo_prior = GeoPrior(embed_dim=embed_dim, num_heads=num_heads)
        
        # Attention 投影
        self.to_q = nn.Conv2d(in_channels, embed_dim, 1)
        self.to_k = nn.Conv2d(in_channels, embed_dim, 1)
        self.to_v = nn.Conv2d(in_channels, embed_dim, 1)
        self.proj = nn.Conv2d(embed_dim, in_channels, 1)
    ...
    def forward(self, x, dsm_img):
        """
        x: RGB Feature Map (B, C, H, W)
        dsm_img: Raw DSM Input (B, 1, H_img, W_img)
        """
        B, C, H, W = x.shape
        L = H * W
        ...
        # 1. 计算几何 Bias
        # shape: (B, Heads, L, L)
        attn_bias = self.geo_prior(x, dsm_img) 
        
        # 2. Self-Attention (With Geo Bias)
        q = self.to_q(x).reshape(B, self.num_heads, C // self.num_heads, L).permute(0, 1, 3, 2) # B, H, L, D
        k = self.to_k(x).reshape(B, self.num_heads, C // self.num_heads, L).permute(0, 1, 3, 2) # B, H, L, D
        v = self.to_v(x).reshape(B, self.num_heads, C // self.num_heads, L).permute(0, 1, 3, 2) # B, H, L, D
        
        # (B, H, L, L)
        attn = (q @ k.transpose(-2, -1)) * self.scale
        
        #  - 这里的 attn_bias 就是几何形状的 mask
        # 关键点：将几何先验注入 Attention
        attn = attn + attn_bias
        
        attn = attn.softmax(dim=-1)
        x_attn = (attn @ v).transpose(1, 2).reshape(B, C, H, W)
...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        # --- [NEW] 添加几何感知辅助头 ---
        # out_c 是 Encoder 输出的通道数 (256)
        # 我们将其挂在最深层的特征上 (包含最丰富的语义信息)
        self.geo_aux_head = GeoAwareAuxHead(in_channels=out_c, num_classes=num_classes, embed_dim=out_c)
        self.aux_loss_weight = 0.4  # 辅助损失的权重
    ...
    def forward(self, x, y, mode='Train'):
        ...
        # DSM 1ch -> 3ch
        y_original = y.clone() # 备份一份原始 DSM 数据用于 Aux Head
        ...
        aux_loss = 0.0
        if mode == 'Train':
            # 我们选择 feats_x[-1] (最深层 RGB 特征) 进行辅助训练
            # 输入: RGB 特征 + 原始 DSM (未归一化的, 或者是归一化前的 tensor)
            # 注意: y_original 通常是 (B, H, W)，需要unsqueeze成 (B, 1, H, W)
            if y_original.dim() == 3:
                y_input = y_original.unsqueeze(1).float()
            else:
                y_input = y_original.float()
            
            # Aux Head Forward
            aux_logits = self.geo_aux_head(feats_x[-1], y_input)
            
            # 双线性插值回原图尺寸用于计算 Loss
            aux_logits = F.interpolate(aux_logits, size=(H_img, W_img), mode='bilinear', align_corners=False)
        ...
        if mode == 'Train':
            return out, align_loss, aux_logits # 返回 aux_logits 供外部计算 CrossEntropy
        else:
            return out
