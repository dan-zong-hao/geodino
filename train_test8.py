...
from UNetFormer_test8 import UNetFormer as MFNet
...
set_seed(SEED)
# 新增：TensorBoard 日志
writer = SummaryWriter(log_dir="./runs/unetformer_test8")
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, dsm, target) in enumerate(train_loader):
            ...
            with autocast(device_type='cuda'):
                output, align_loss, aux_logits = net(data, dsm, mode='Train')
                λ = 0.1
                # loss = criterion(output, target) + λ * align_loss
                # loss = criterion(output, target)
                λ_aux = 0.4 
                loss_aux = loss_calc(aux_logits, target, weights)
                loss = loss_calc(output, target, weights) + λ * align_loss
                loss = loss + λ_aux * loss_aux
        ...
        if e % save_epoch == 0:
            ...
            writer.add_scalar("Loss_Components/aux", epoch_aux, e)
            test_time = time.time()
            
            print("Test time: {:.3f} seconds".format(test_time - train_time))
            if MIoU > MIoU_best:
                if DATASET == 'Vaihingen':
                    torch.save(net.state_dict(), './resultsv_test8/{}_epoch{}_{}'.format(MODEL, e, MIoU))
