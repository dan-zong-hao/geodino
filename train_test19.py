...
set_seed(SEED)
# 新增：TensorBoard 日志
writer = SummaryWriter(log_dir="./runs/unetformer_test19")
...
scaler = GradScaler()
max_grad_norm = 5.0
def train(net, optimizer, epochs, scheduler=None, weights=WEIGHTS, save_epoch=1, mmst_prob=0.5):
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, dsm, target) in enumerate(train_loader):
            ...
            # ========= 新增：本 batch 是否触发 MMST =========
            # 只有当模型本身开启 use_MMST 且抽样命中时才触发
            mmst_trigger = (net.use_MMST and (torch.rand((), device=data.device) < mmst_prob))

            # 为了避免不触发时还跑两套 decoder，临时切换 use_MMST
            orig_use_mmst = net.use_MMST
            net.use_MMST = mmst_trigger

            try:
                with autocast(device_type='cuda'):
                    if net.use_MMST:
                        logits, logits_rgb, logits_dsm, align_loss = net(data, dsm, mode='Train')
                    else:
                        logits, align_loss = net(data, dsm, mode='Train')
                        logits_rgb, logits_dsm = None, None

                    loss_main = loss_calc(logits, target, weights)

                    if net.use_MMST:
                        with torch.no_grad():
                            pred = logits.softmax(dim=1).argmax(dim=1)
                            pred_cor = (pred == target) & (target != 255)
                            mask_lbl = target.clone()
                            mask_lbl[~pred_cor] = 255

                        loss_rgb = loss_calc(logits_rgb, mask_lbl, weights)
                        loss_dsm = loss_calc(logits_dsm, mask_lbl, weights)
                        loss = loss_main + mmst_w * (loss_rgb + loss_dsm) + align_w * align_loss
                    else:
                        loss_rgb = torch.tensor(0.0, device=logits.device)
                        loss_dsm = torch.tensor(0.0, device=logits.device)
                        loss = loss_main + align_w * align_loss
                ...
                # ========= 恢复模型原始开关，避免影响后续 batch / eval =========
                net.use_MMST = orig_use_mmst
            ...
            if iter_ % 100 == 0:
                ...
                print(
                    ...
                    f'TriggerMMST: {int(mmst_trigger)} (p={mmst_prob})\t'
                    f'Loss: {loss.item():.4f} (Main: {loss_main.item():.4f}, '
                    f'RGB: {loss_rgb.item():.4f}, DSM: {loss_dsm.item():.4f}, Align: {float(align_loss):.4f})'
                )
        ...
        if e % save_epoch == 0:
            ...
            if MIoU > MIoU_best:
                if DATASET == 'Vaihingen':
                    torch.save(net.state_dict(), './resultsv_test19/{}_epoch{}_{}'.format(MODEL, e, MIoU))
