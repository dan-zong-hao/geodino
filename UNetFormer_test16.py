...
class GeometryPriorGenerator(nn.Module):
    """
    Test17: 动态门控 (Dynamic Gating Only)
    - 特征: MaxPool + Roughness [Test14]
    - 机制: 所有 16 个 Head 都启用，但权重 Alpha 由 MobileNetV3 动态预测
    """
    def __init__(self, input_size=256, patch_size=14, num_heads=16):
        ...
        # --- B. Gating Net (MobileNetV3-Small) ---
        # 使用 MobileNetV3-Small，比 V2 更轻量，适合做 Gating
        mbv3 = mobilenet_v3_small(weights=MobileNet_V3_Small_Weights.DEFAULT)
        
        # 我们只需要浅层/中层语义来判断纹理复杂度，截取前 4 个 stage 即可
        # MobileNetV3 的 features 是 Sequential，可以直接切片
        # 这会极大减少计算量
        self.gating_backbone = nn.Sequential(*list(mbv3.features.children())[:4])
        
        # 冻结参数
        for p in self.gating_backbone.parameters():
            p.requires_grad = False
            
        # 动态获取输出通道数 (Dummy Forward)
        with torch.no_grad():
            dummy = torch.randn(1, 3, 256, 256)
            out_channels = self.gating_backbone(dummy).shape[1]

        # 适配头
        self.gating_head = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Flatten(),
            nn.Linear(out_channels, num_heads * 3) # 为所有 Head 预测 3 个系数
        )
        
        # 零初始化策略 (至关重要)
        # 初始 Bias 设为 -3.0，Sigmoid(-3) ≈ 0.04
        # 保证训练初期 Alpha 接近 0，退化回 Test13 Baseline
        nn.init.constant_(self.gating_head[-1].weight, 0.0)
        nn.init.constant_(self.gating_head[-1].bias, -3.0)

    def forward(self, dsm_map, rgb_map):
        """
        需要同时传入 dsm_map 和 rgb_map
        """
        B = dsm_map.shape[0]
        
        # 1. 动态生成 Alpha
        with torch.no_grad():
            feat = self.gating_backbone(rgb_map)
        
        gating_logits = self.gating_head(feat)
        alphas = torch.sigmoid(gating_logits).view(B, self.num_heads, 3, 1, 1)
        
        alpha_spatial = alphas[:, :, 0]
        ...
        # 3. 计算差异
        depth_dist = torch.abs(h_tokens - h_tokens.transpose(1, 2)).unsqueeze(1)
        rough_dist = torch.abs(r_tokens - r_tokens.transpose(1, 2)).unsqueeze(1)

        # 4. 融合
        # 这里的 Alpha 是动态变化的，且对所有 Head 生效
        bias = - (alpha_spatial * self.spatial_dist + 
