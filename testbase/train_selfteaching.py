...
from UNetFormer_test2 import UNetFormer as MFNet
...
set_seed(SEED)
# 新增：TensorBoard 日志
writer = SummaryWriter(log_dir="./runs/unetformer_selfTeaching")
...
lora_params, head_params, norm_params, gating_params = [], [], [], []

for n, p in net.named_parameters():
    if not p.requires_grad:
        continue
    
    # [修改点] 优先判断是否属于 Gating Net
    # 根据你的模块命名，名字里通常包含 'geo_gen.gating_net'
    if 'geo_gen.gating_net' in n:
        gating_params.append(p)
    elif is_lora_param(n):
        lora_params.append(p)
...
optimizer = optim.AdamW(
    [
        ...
        {'params': lora_params, 'lr': 3e-4, 'weight_decay': 0.0},   # LoRA略高lr
        # [修改点] 直接使用筛选好的列表，不要再调用 .parameters()
        {'params': gating_params, 'lr': 1e-3, 'weight_decay': 1e-2} # Gating Net 大学习率
    ],
    betas=(0.9, 0.999)
)
...
def calc_self_teaching_loss(pred_student, target, mask_correct):
    """
    pred_student: 单模态分支的预测输出 (B, C, H, W)
    target: 真实标签 (B, H, W)
    mask_correct: 双模态预测正确的区域掩码 (B, H, W), bool 类型
    """
    # 1. 计算标准的 CE Loss (不进行 reduction，保留像素级 loss)
    # ignore_index=255 应该与你数据集设置一致
    ce_loss = F.cross_entropy(pred_student, target, ignore_index=255, reduction='none')
    
    # 2. 只保留 Teacher 预测正确的区域
    # mask_correct 是 bool，转换为 float 进行乘法
    valid_loss = ce_loss * mask_correct.float()
    
    # 3. 计算平均 Loss
    # 加上 1e-6 防止除以 0 (如果没有预测正确的区域)
    loss = valid_loss.sum() / (mask_correct.float().sum() + 1e-6)
...
def train(net, optimizer, epochs, scheduler=None, weights=WEIGHTS, save_epoch=1):
    ...
    # [新增] 辅助损失权重
    lambda_aux = 0.5 
    # [新增] 自教学触发概率 (例如 0.5 表示 50% 的 batch 会触发 MMST)
    prob_mmst = 0.5
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, dsm, target) in enumerate(train_loader):
            ...
            # ==========================================
            # 1. [Teacher Step] 全模态前向传播 (始终执行)
            # ==========================================
            with autocast(device_type='cuda'):
                output, align_loss = net(data, dsm, mode='Train')
                λ = 0.0
                # 计算主任务 Loss
                loss_main = loss_calc(output, target, weights) + λ * align_loss

            # 初始化自教学损失为 0 (如果没触发概率，则不产生梯度)
            loss_rgb = torch.tensor(0.0, device=data.device)
            loss_dsm = torch.tensor(0.0, device=data.device)

            # ==========================================
            # [Probabilistic Trigger] 概率触发自教学逻辑
            # ==========================================
            # 只有满足概率要求时，才执行 Mask 生成和 Student 前向传播
            if random.random() < prob_mmst:
                
                # 2. [Mask Generation] 生成“正确预测掩码”
                with torch.no_grad():
                    pred_teacher = torch.argmax(output, dim=1)
                    mask_correct = (pred_teacher == target) & (target != 255)

                # 3. [Student Step 1] 遮盖 DSM (训练 RGB 分支)
                with autocast(device_type='cuda'):
                    dsm_zeros = torch.zeros_like(dsm)
                    output_rgb, _ = net(data, dsm_zeros, mode='Train')
                    loss_rgb = calc_self_teaching_loss(output_rgb, target, mask_correct)

                # 4. [Student Step 2] 遮盖 RGB (训练 DSM 分支)
                with autocast(device_type='cuda'):
                    data_zeros = torch.zeros_like(data)
                    output_dsm, _ = net(data_zeros, dsm, mode='Train')
                    loss_dsm = calc_self_teaching_loss(output_dsm, target, mask_correct)

            # ==========================================
            # 5. 总损失与反向传播
            # ==========================================
            # 如果触发了 MMST，loss_rgb 和 loss_dsm 会有值；否则为 0
            loss = loss_main + lambda_aux * (loss_rgb + loss_dsm)
        ...
        if e % save_epoch == 0:
            ...
            if MIoU > MIoU_best:
                if DATASET == 'Vaihingen':
                    torch.save(net.state_dict(), './resultsv_selfTeaching/{}_epoch{}_{}'.format(MODEL, e, MIoU))
