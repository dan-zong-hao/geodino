...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        # 2) PEFT LoRA 配置与注入 (默认全层)
        if use_lora:
            # 定义 LoRA 配置
            peft_config = LoraConfig(
                r=lora_rank,
                lora_alpha=lora_alpha,
                # 直接传入 target 后缀列表，PEFT 会自动匹配模型中所有层对应的模块
            ...
            # --- 注入双 Adapter (RGB & DSM) ---
            # 1. 创建第一个 Adapter，命名为 'rgb'
            # 这会自动冻结原模型参数，并激活 'rgb' adapter
            self.image_encoder = get_peft_model(self.image_encoder, peft_config, adapter_name="rgb")
            
            # 2. 添加第二个 Adapter，命名为 'dsm'
            # 使用相同的配置，初始化一套独立的 LoRA 权重
            self.image_encoder.add_adapter("dsm", peft_config)
            for name, param in self.image_encoder.named_parameters():
                if "lora_" in name: # 只要是 LoRA 参数
                    param.requires_grad = True
            # 覆盖 freeze_backbone 逻辑，因为 peft 已经帮我们处理了冻结
            freeze_backbone = False
            
            print(f"LoRA enabled on all layers with targets: {lora_targets}")
            self.image_encoder.print_trainable_parameters()

        self.transform = self.make_transform(256)

        # 根据模型调整维度 (ViT-L=1024)
        # 建议动态获取，或者手动根据你使用的模型修改
        dinov3_out = self.image_encoder.config.hidden_size 
        
        
        out_c = 256
        encoder_channels = (out_c, out_c, out_c, out_c)

        # 2) 线性投影 & 层选择 (ViT-H+ 32层建议选 [7, 15, 23, 31])
        # 如果是 ViT-H+ (32 layers)
        if hasattr(self.image_encoder.config, 'num_hidden_layers') and self.image_encoder.config.num_hidden_layers == 32:
            self.layer_ids = [7, 15, 23, 31]
        else:
            # 默认 ViT-L (24 layers)
    ...
    def forward(self, x, y, mode='Train'):
        ...
        # 取 hidden_states
        self.image_encoder.set_adapter("rgb")
        out_x = self.image_encoder(x, output_hidden_states=True)
        self.image_encoder.set_adapter("dsm")
