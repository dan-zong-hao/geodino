...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        cfg_hf = self.image_encoder.config
        hidden = getattr(cfg_hf, "hidden_size", 1024)
        num_layers = getattr(cfg_hf, "num_hidden_layers", 24)
        num_regs = getattr(cfg_hf, "num_register_tokens", 4)
        self._num_special_tokens = 1 + num_regs
        if use_lora:
            last_k = min(lora_last_k, num_layers)
            lora_layers = list(range(num_layers - last_k, num_layers))
    ...
    def _tokens_to_map(self, toks, proj):
        """
        toks: [B, T, C]  -> 去特殊token -> [B, N, C] -> Linear投影 -> [B, N, 256]
           -> reshape -> [B,256,H,W]，H=W=16（输入256、patch16）
        """
        toks, _ = self._strip_special_tokens(toks)   # [B, N, C]
        x = proj(toks)                               # [B, N, 256]
        B, N, C = x.shape
        H = int(math.isqrt(N))
        assert H * H == N, "Patch tokens不能reshape成方阵，请检查输入尺寸/特殊token数"
        x = x.reshape(B, H, H, C).permute(0, 3, 1, 2).contiguous()  # [B,256,16,16]
        return x
