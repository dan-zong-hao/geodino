...
from loss_test1 import CombinedLoss
try:
...
set_seed(SEED)
# 新增：TensorBoard 日志
writer = SummaryWriter(log_dir="./runs/unetformer_test4")
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, dsm, target) in enumerate(train_loader):
            ...
            with autocast(device_type='cuda'):
                ...
                λ = 0.1
                loss_seg, loss_dict = criterion(output, target)
                loss = loss_seg + λ * align_loss
        ...
        if e % save_epoch == 0:
            ...
            epoch_focal /= len(train_loader)
            epoch_dice /= len(train_loader)
            epoch_align /= len(train_loader)
            net.eval()
            ...
            writer.add_scalar("Loss_Components/Focal", epoch_focal, e)
            writer.add_scalar("Loss_Components/Dice", epoch_dice, e)
            writer.add_scalar("Loss_Components/InfoNCE", epoch_align, e)
            test_time = time.time()
            
            print("Test time: {:.3f} seconds".format(test_time - train_time))
            if MIoU > MIoU_best:
                if DATASET == 'Vaihingen':
                    torch.save(net.state_dict(), './resultsv_test4/{}_epoch{}_{}'.format(MODEL, e, MIoU))
