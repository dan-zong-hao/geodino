...
class Adapter(nn.Module):
    def __init__(self, D_features, mlp_ratio=0.25, act_layer=nn.GELU, skip_connect=True):
        ...
        with torch.no_grad():
            ...
            nn.init.zeros_(self.D_fc_prompt.weight) # Prompt初始化为0，初始不影响
            nn.init.zeros_(self.D_fc_prompt.bias)
...
# --- 2. 基于 MAPA 的投影层 ---
class MAPAProjector(nn.Module):
    def __init__(self, in_dim=1024, out_dim=256):
        super().__init__()
        
        # 1. 基础降维 (Linear 1024 -> 256)
        # 因为 DINOv3 输出是 Token，这里直接用 Linear 更顺手
        self.proj_rgb = nn.Linear(in_dim, out_dim)
        self.proj_dsm = nn.Linear(in_dim, out_dim)
        self.norm = nn.LayerNorm(out_dim) # 统一归一化
        
        # 2. 统一表征生成 (Unified Representation)
        # 论文公式: X_uni = W_uni * Concat(X_rgb, X_x)
        self.unified_merger = nn.Sequential(
            nn.Linear(out_dim * 2, out_dim // 2),
            nn.GELU(),
            nn.Linear(out_dim // 2, out_dim)
        )
        
        # 3. 提示生成 (Prompt Generation)
        # 论文公式: P = Gamma * X_uni + Beta (这里简化为直接投影)
        self.prompt_gen_rgb = nn.Linear(out_dim, out_dim)
        self.prompt_gen_dsm = nn.Linear(out_dim, out_dim)
        
        # 4. 适配器 (使用上面的 Adapter 类)
        # mlp_ratio 可以设小一点(如0.25)以节省参数
        self.adapter_rgb = Adapter(out_dim, mlp_ratio=0.25)
        self.adapter_dsm = Adapter(out_dim, mlp_ratio=0.25)

    def forward(self, x_rgb, x_dsm):
        """
        输入: [B, N, 1024] 的 Token 序列
        输出: [B, 256, H, W] 的特征图
        """
        # 1. 剥离 CLS Token (如果你的输入包含 CLS)
        # 假设 x_rgb[:, 0] 是 CLS, 后面是 Patch
        # 这里为了简化，假设传入前已经剥离好了，或者直接处理 Patch 部分
        B, N, C = x_rgb.shape
        H = int(math.isqrt(N)) 
        
        # 2. 基础降维
        feat_rgb = self.proj_rgb(x_rgb) # [B, N, 256]
        feat_dsm = self.proj_dsm(x_dsm)
        feat_rgb = self.norm(feat_rgb)
        feat_dsm = self.norm(feat_dsm)
        
        # 3. 生成统一表征 X_uni
        # 拼接 RGB 和 DSM
        cat_feat = torch.cat([feat_rgb, feat_dsm], dim=-1) # [B, N, 512]
        x_uni = self.unified_merger(cat_feat) # [B, N, 256]
        
        # 4. 生成 Prompt
        p_rgb = self.prompt_gen_rgb(x_uni)
        p_dsm = self.prompt_gen_dsm(x_uni)
        
        # 5. MAPA 适配 (注入 Prompt 进行校准)
        out_rgb = self.adapter_rgb(feat_rgb, prompt=p_rgb)
        out_dsm = self.adapter_dsm(feat_dsm, prompt=p_dsm)
        
        # 6. Reshape 成图片 [B, N, C] -> [B, C, H, W]
        out_rgb = out_rgb.transpose(1, 2).reshape(B, -1, H, H)
        out_dsm = out_dsm.transpose(1, 2).reshape(B, -1, H, H)
...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        # 替换原来的 Linear 定义
        # self.proj_rgb = nn.ModuleList([Projector(1024, 256) for _ in range(4)])
        # self.proj_dsm = nn.ModuleList([Projector(1024, 256) for _ in range(4)])
        self.projectors = nn.ModuleList([
            MAPAProjector(dinov3_out, out_c) for _ in range(4)
        ])
    ...
    def _tokens_to_map(self, toks, projector):
        """
        toks: [B, T, C] (DINOv3 输出的原始 Token 序列)
        projector: Projector 实例 (定义了从 1024 -> 256 的转换)
        """
        # 1. 剥离 CLS 等特殊 Token，只保留 Patch Tokens
        # 返回 toks: [B, N, 1024]
        toks, _ = self._strip_special_tokens(toks) 
        
        # 2. 直接调用 Projector
        # Projector 内部会处理 [B, N, C] -> [B, C, H, W] 的变换和卷积
        x = projector(toks) 
    ...
    def forward(self, x, y, mode='Train'):
        ...
        feats_x, feats_y = [], []
        for i, lid in enumerate(self.layer_ids):
            toks_x = out_x.hidden_states[lid]
            toks_y = out_y.hidden_states[lid]

            patch_x, _ = self._strip_special_tokens(toks_x)
            patch_y, _ = self._strip_special_tokens(toks_y)

            fx, fy = self.projectors[i](patch_x, patch_y)
