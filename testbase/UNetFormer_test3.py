...
class Projector(nn.Module):
    def __init__(self, in_dim=1024, out_dim=256):
        super().__init__()
        # 1. 先用 1x1 卷积降维 (替代 Linear)
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_dim, out_dim, 1),
            nn.BatchNorm2d(out_dim), # 或 LayerNorm
            nn.GELU()
        )
        # 2. 再接一个 3x3 深度卷积 (DW-Conv) 补充空间信息
        # 这一步能补偿降维带来的空间信息损失
    ...
    def forward(self, x):
        # x: [B, N, C] -> 需要先 reshape 成图片
        B, N, C = x.shape
        H = int(math.isqrt(N))
        # [B, H, W, C] -> [B, C, H, W]
        x_img = x.transpose(1, 2).view(B, C, H, H)
        
        x_out = self.conv1(x_img)
        x_out = x_out + self.conv2(x_out) # 残差连接
