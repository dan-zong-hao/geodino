...
from UNetFormer_test22 import UNetFormer as MFNet
...
set_seed(SEED)
# 新增：TensorBoard 日志
writer = SummaryWriter(log_dir="./runs/unetformer_test22")
    ...
    with torch.no_grad():
        for img, dsm, gt, gt_e in tqdm(zip(test_images, test_dsms, test_labels, eroded_labels), total=len(test_ids), leave=False):
            ...
            for i, coords in enumerate(
                ...
                # Do the inference
                outs, align_loss, _, __ = net(image_patches, dsm_patches, mode='Test')
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, dsm, target) in enumerate(train_loader):
            ...
            with autocast(device_type='cuda'):
                ...
                # 1. Teacher Pass (完整输入)
                # ==========================================
                out_teacher, _, _, align_loss = net(data, dsm, mode='Train', student_mode=None)
                
                loss_main = loss_calc(out_teacher, target, weights)
                
                loss_rgb = torch.tensor(0.0, device=data.device)
                loss_dsm = torch.tensor(0.0, device=data.device)

                # ==========================================
                # 2. Student Passes (Partial Masking)
                # ==========================================
                if use_mmst_step:
                    # A. 制作 Teacher Label
                    with torch.no_grad():
                        pred_teacher = out_teacher.detach().softmax(dim=1).argmax(dim=1)
                        pred_cor = (pred_teacher == target)
                        mask_lbl = target.clone()
                        mask_lbl[~pred_cor] = ignore_label
                    
                    # B. Student RGB Pass (Key Fix for Test21)
                    # 调用 student_mode='rgb'，此时 net 会保留 geo-bias 但 mask 掉 DSM features
                    out_student_rgb, _, _, _ = net(data, dsm, mode='Train', student_mode='rgb')
                    loss_rgb = loss_calc(out_student_rgb, mask_lbl, weights)

                    # C. Student DSM Pass
                    # 调用 student_mode='dsm'
                    out_student_dsm, _, _, _ = net(data, dsm, mode='Train', student_mode='dsm')
                    loss_dsm = loss_calc(out_student_dsm, mask_lbl, weights)
