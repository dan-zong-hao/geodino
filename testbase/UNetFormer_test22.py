...
class UNetFormer(nn.Module):
    ...
    def forward(self, x, y, mode='Train', student_mode=None):
        """
        student_mode: 
            None  -> 正常 Teacher 模式 (RGB + DSM)
            'rgb' -> RGB Student 模式 (RGB + Masked DSM Features + Valid Geo-Bias)
            'dsm' -> DSM Student 模式 (Masked RGB + DSM)
        """
        
        # 1. 生成几何先验 Bias (关键修改)
        # 无论 student_mode 是什么，Geo-Gen 始终接收真实的 y (DSM)。
        # 这保证了 RGB Encoder 内部的 Attention 机制不会崩溃。
        # 只有在非常极端的情况下（如我们想测试无先验的效果）才屏蔽这里，但为了“部分屏蔽”策略，这里不屏蔽。
        geo_bias = self.geo_gen(y.unsqueeze(1)) 
        
        # 2. 准备输入数据 (实施特征级屏蔽)
        # DSM: (B, H, W) -> (B, 3, H, W)
        y_in_raw = torch.unsqueeze(y, dim=1).repeat(1, 3, 1, 1)
        x_in_raw = x

        if student_mode == 'rgb':
            # 【策略核心】：RGB Student 模式
            # 1. Geo-Bias 正常生成 (上一步已做)
            # 2. 进入 image_encoder 的 DSM 像素被置零 -> 强迫不依赖 DSM 特征
            y_in_raw = torch.zeros_like(y_in_raw)
            
        elif student_mode == 'dsm':
            # DSM Student 模式
            # 屏蔽 RGB 输入
            x_in_raw = torch.zeros_like(x_in_raw)
            # 注意：DSM Student 理论上只靠 y_in_raw 工作，所以 x_in_raw 置零没问题
        
        # 3. 预处理 (Transform)
        # 此时已经完成了 Masking，Transform 会处理全 0 数据 (Normalize 后变为非零常数，这是合理的)
        x_in = self.transform(x_in_raw)
        y_in = self.transform(y_in_raw)
        ...
        # --- Pass 1: RGB 分支 ---
        for m in att_layers:
            m.geometry_bias = geo_bias # 挂载 Geo-Bias
        
        try:
            if hasattr(self.image_encoder, 'set_modality'): 
                self.image_encoder.set_modality('rgb')
            
            # 如果是 'dsm' 模式，x_in 是全0，这里算出的特征也是废的，但不影响程序运行
            # 如果是 'rgb' 模式，x_in 是真图，geo_bias 是真 Bias，计算出的特征是“受几何引导的RGB特征”
            out_x = self.image_encoder(x_in, output_hidden_states=True)
            
        finally:
            for m in att_layers:
                m.geometry_bias = None # 卸载 Bias

        # --- Pass 2: DSM 分支 ---
        # 此时 Bias 已卸载，DSM 分支是纯 ViT
        if hasattr(self.image_encoder, 'set_modality'): 
            self.image_encoder.set_modality('dsm')
            
        # 如果是 'rgb' 模式，y_in 是全0，DSM 分支输出无意义特征
        # 如果是 'dsm' 模式，y_in 是真图，正常输出
        out_y = self.image_encoder(y_in, output_hidden_states=True)

        # 5. FPN & Fusion & Decoder (保持不变)
        feats_x, feats_y = [], []
        ...
        # Align Loss 计算 (仅在 Teacher 模式下计算才有意义，或者由外部控制)
        # 为了简单，我们始终计算，但在 Student 模式下这个 Loss 可能很大或无意义，训练脚本应只取 Teacher 的 align_loss
        align_loss = torch.tensor(0.0, device=x.device)
        # 这里你可以选择是否计算，或者仅在 student_mode is None 时计算
        ...
        return out, align_loss, None, None # 保持接口一致，后两个返回值不再作为 Aux Head，留空
