...
class GeometryPriorGenerator(nn.Module):
    """
    修正版 GeoPrior (FINAL FIXED VERSION)：
    1. 移除无效的 Bias 参数 (bias_spatial, bias_depth, bias_rough)。
    2. 使用 softplus 强制 temperature 为正。
    3. 优化归一化逻辑，避免 Batch 统计量相互干扰。
    """
    ...
    def forward(self, dsm_map):
        ...
        # [优化] 单样本独立归一化 (Per-sample Normalization)
        # 在 (N, N) 维度上找最大值，保持 (B, 1, 1, 1) 的形状进行广播除法
        # 这样避免了 Batch 内某个极端样本拉低其他样本的数值范围
        depth_max = depth_dist.flatten(2).max(dim=2)[0].view(B, 1, 1, 1)
        depth_dist = depth_dist / (depth_max + 1e-6)

        rough_max = rough_dist.flatten(2).max(dim=2)[0].view(B, 1, 1, 1)
        rough_dist = rough_dist / (rough_max + 1e-6)

        # 4. 计算 softplus 激活后的参数
        alpha_spatial = F.softplus(self.log_alpha_spatial)
        alpha_depth   = F.softplus(self.log_alpha_depth)
        alpha_rough   = F.softplus(self.log_alpha_rough)
        
        # 5. 融合先验 (移除无效 Bias)
        bias = (
            alpha_spatial * self.spatial_dist + 
            alpha_depth * depth_dist + 
            alpha_rough * rough_dist
        )
        
        # 6. 应用温度参数 (强制为正) 并取负
        # 使用 softplus 确保 temperature > 0，防止先验逻辑反转
        valid_temp = F.softplus(self.temperature)
        bias = -bias * valid_temp
        
        # 扩展到 batch 维度 (spatial_dist 是 buffer 需要 expand，其他已经是 B 开头了)
        if bias.shape[0] != B:
