...
class UNetFormer(nn.Module):
    def __init__(self,
        ...
        self.layer_ids_rgb = [5, 11, 17, 23]
        self.layer_ids_dsm = [2, 5, 8, 11]
    ...
    def forward(self, x, y, mode='Train'):
        ...
        #     feats_y.append(fy)
        feats_x, feats_y = [], []
        for i, (lid_rgb, lid_dsm) in enumerate(zip(self.layer_ids_rgb, self.layer_ids_dsm)):
            fx = self._tokens_to_map_with_bn(out_x.hidden_states[lid_rgb],
                                            self.proj_shared[i], self.bn_rgb[i])
            fy = self._tokens_to_map_with_bn(out_y.hidden_states[lid_dsm],
