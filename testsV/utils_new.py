...
def build_fast_val_ids(ids, k=200, seed=666):
    """对 VALID 做 scene×alt 分层抽样；其它数据集随机抽样"""
    ...
    if DATASET != "VALID":
        return random.sample(list(ids), k)

    # VALID：按 label json 的 file_name 解析 scene/alt 分层抽样
    groups = defaultdict(list)
    for name in ids:
        jname = name if name.endswith(".json") else (name + ".json")
        jpath = os.path.join(MAIN_FOLDER, "label", jname)
        with open(jpath, "r", encoding="utf-8") as f:
            meta = json.load(f)

        fn = meta["file_name"].replace("\\", "/")  # images/<scene>/<alt>/xxx.png
        parts = fn.split("/")
        scene = parts[1] if len(parts) > 1 else "unknown"
        alt   = parts[2] if len(parts) > 2 else "unknown"
        groups[(scene, alt)].append(name)

    keys = sorted(groups.keys())
    G = len(keys)
    base = max(1, k // max(1, G))
    ...
    for key in keys:
        lst = groups[key]
        random.shuffle(lst)
        picked.extend(lst[:min(base, len(lst))])
    ...
    if remain > 0:
        ...
        for key in keys:
            pool.extend([x for x in groups[key] if x not in picked_set])
        random.shuffle(pool)
        picked.extend(pool[:remain])
...
if DATASET == 'Vaihingen':
    ...
    train_ids = _read_valid_split("seg_train.txt")
    val_ids   = _read_valid_split("seg_val.txt")
...
class ISPRS_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        # -------------------------
        if DATASET == "VALID":
            # 1) 读 RGB（uint8 HWC），用它的 H/W 决定裁块位置
            #    注意：不急着转 float/CHW，先裁 patch 再转换更省时间/内存
            if (self.cache and random_idx in self.data_cache_):
                img = self.data_cache_[random_idx]  # uint8 HWC
            else:
                img = np.array(Image.open(self.data_files[random_idx]))  # HWC, uint8
                if img.ndim == 3 and img.shape[2] >= 4:
                    img = img[:, :, :3]
                if self.cache:
                    self.data_cache_[random_idx] = img

            H, W = img.shape[0], img.shape[1]
            ph, pw = WINDOW_SIZE
            # 防御：避免尺寸刚好等于 patch 时 randint 报错
            if H <= ph or W <= pw:
                x1, y1 = 0, 0
            else:
                x1 = random.randint(0, H - ph - 1)
                y1 = random.randint(0, W - pw - 1)
            x2, y2 = x1 + ph, y1 + pw

            # 2) RGB patch -> float32 CHW [0,1]
            data_p = img[x1:x2, y1:y2, :].transpose(2, 0, 1).astype(np.float32) / 255.0

            # 3) depth：读整张（扁平目录），但只对 patch 做 /256 + minmax
            if (self.cache and random_idx in self.dsm_cache_):
                dep = self.dsm_cache_[random_idx]  # float32 2D（可选缓存整张，建议 VALID 训练时 cache=False）
            else:
                dep_u16 = np.array(Image.open(self.dsm_files[random_idx]))  # 2D, uint16
                dep = dep_u16.astype(np.float32) / 256.0
                if self.cache:
                    self.dsm_cache_[random_idx] = dep

            dsm_p = dep[x1:x2, y1:y2]
            mn, mx = float(dsm_p.min()), float(dsm_p.max())
            dsm_p = (dsm_p - mn) / (mx - mn + 1e-8)

            # 4) semantic：先裁 patch，再 convert_from_color（核心提速点）
            if (self.cache and random_idx in self.label_cache_):
                lab_rgb = self.label_cache_[random_idx]  # uint8 HWC（整张）
            else:
                lab_rgb = np.array(Image.open(self.label_files[random_idx]))  # HWC, uint8
                if lab_rgb.ndim == 3 and lab_rgb.shape[2] >= 4:
                    lab_rgb = lab_rgb[:, :, :3]
                if self.cache:
                    self.label_cache_[random_idx] = lab_rgb

            lab_patch = lab_rgb[x1:x2, y1:y2, :]
            label_p = convert_from_color(lab_patch).astype(np.int64)

            # 5) Augment（复用你原来的翻转/镜像）
...
def loss_calc(pred, label, weights):
    ...
    # 1. 先实例化 Loss 类（这里只传配置参数，不传数据）
    # 如果你的 ignore_label 不是默认的 255，可以在这里修改
    criterion = CrossEntropy2d_ignore(ignore_label=255).cuda()
    
    # 2. 调用实例化的对象，这会自动触发 forward 方法（这里传数据）
