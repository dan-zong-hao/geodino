...
class GeometryPriorGenerator(nn.Module):
    """
    改进版 GeoPrior：
    1. 混合池化 (Max + Avg) 以锐化边缘 (针对 Tree vs LowVeg)
    2. 粗糙度感知 (Std) 以区分纹理 (针对 Road vs LowVeg)
    3. 零初始化 (Zero Init) 保证训练稳定性
    """
    def __init__(self, input_size=256, patch_size=14, num_heads=16):
        ...
        # --- B. 可学习参数 (改为 0 初始化) ---
        # 即使初始为0，梯度也会让它迅速学到非0值，但起点更安全
        self.alpha_spatial = nn.Parameter(torch.zeros(1, num_heads, 1, 1)) 
        self.alpha_depth   = nn.Parameter(torch.zeros(1, num_heads, 1, 1))
        self.alpha_rough   = nn.Parameter(torch.zeros(1, num_heads, 1, 1)) # 新增：粗糙度权重
    ...
    def forward(self, dsm_map):
        ...
        # 1. DSM 特征提取 (针对性改进)
        # ------------------------------------------------------------------
        # 分支 A: 平均高度 (平滑特征，适合大面积地形)
        dsm_avg = F.adaptive_avg_pool2d(dsm_map, (self.H_grid, self.W_grid))
        
        # 分支 B: 最大高度 (锐利特征，针对 Tree/Building 边缘) [关键改进!]
        dsm_max = F.adaptive_max_pool2d(dsm_map, (self.H_grid, self.W_grid))
        
        # 分支 C: 局部粗糙度 (标准差，针对 Low Veg vs Road) [关键改进!]
        # 近似计算：在每个 Patch 窗口内计算 std
        # 简单做法：利用 AvgPool 模拟: E[X^2] - (E[X])^2
        dsm_sq = dsm_map ** 2
        avg_sq = F.adaptive_avg_pool2d(dsm_sq, (self.H_grid, self.W_grid))
        # std = sqrt(avg(x^2) - avg(x)^2)，加 epsilon 防止 NaN
        dsm_std = torch.sqrt(torch.clamp(avg_sq - dsm_avg ** 2, min=1e-6))

        # ------------------------------------------------------------------

        # 2. 准备 Tokens [B, N, 1]
        # 这里我们将 Max 和 Avg 融合 (简单相加或取最大，这里取 Max 更激进，强调边缘)
        # 或者为了保留两种信息，我们可以让 depth_dist 综合两者
        # 策略：主要用 MaxPool 代表“物体高度”，因为树冠是覆盖在草地上的
