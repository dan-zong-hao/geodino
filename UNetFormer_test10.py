...
class GeometryPriorGenerator(nn.Module):
    ...
    def __init__(self, input_size=256, patch_size=14, num_heads=16):
        ...
        # --- B. 可学习衰减参数 ---
        self.alpha_spatial = nn.Parameter(torch.ones(1, num_heads, 1, 1) * 0.1)
        self.alpha_depth   = nn.Parameter(torch.ones(1, num_heads, 1, 1) * 0.5)

    def forward(self, dsm_map):
        """
        dsm_map: [B, 1, H_img, W_img] (必须是 4D)
        """
        # 1. 下采样到 Patch Token 尺度
        dsm_small = F.adaptive_avg_pool2d(dsm_map, (self.H_grid, self.W_grid))
        # [B, N, 1]
        dsm_tokens = dsm_small.flatten(2).transpose(1, 2)

        # 2. 计算深度差 (Depth Prior) [B, 1, N, N]
        depth_dist = torch.abs(dsm_tokens - dsm_tokens.transpose(1, 2)).unsqueeze(1)

        # 3. 融合 (Logit Bias = - (alpha * dist))
        bias = - (torch.abs(self.alpha_spatial) * self.spatial_dist + 
                  torch.abs(self.alpha_depth) * depth_dist)
...
class GeometryAwareAttention(nn.Module):
    """
    包装器：包裹原始的 Attention 层。
    作用：拦截 forward 调用，自动填充并注入 geometry_bias。
    """
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False, **kwargs):
        ...
        if geo_bias is not None:
            # --- 关键修复：处理 Special Tokens (CLS + Registers) ---
            # hidden_states shape: [Batch, Seq_Len, Dim]
            B, seq_len, _ = hidden_states.shape
            # geo_bias shape: [B, Heads, H_grid*W_grid, H_grid*W_grid] (例如 256x256)
            num_patches = geo_bias.shape[-1]
            
            # 如果序列长度 > Patch 数量，说明存在 CLS/Register tokens
            if seq_len > num_patches:
                num_special_tokens = seq_len - num_patches
                # DINOv3 将特殊 token 放在序列开头，因此我们在 左侧 和 上侧 填充 0
                # F.pad 参数顺序: (padding_left, padding_right, padding_top, padding_bottom)
                # 填充 0 表示这些特殊 token 与 patch 之间没有几何距离惩罚（允许全局 Attention）
            ...
            if attention_mask is not None:
                # 兼容 bool 类型的 mask (如 padding mask)
...
class UNetFormer(nn.Module):
    ...
    def forward(self, x, y, mode='Train'):
        ...
        # 此处省略了 info_nce 的计算代码，如需使用请取消注释并确保 local_infonce 方法存在
        align_loss = torch.tensor(0.0).to(x.device) 
